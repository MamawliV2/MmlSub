...
admins = [865122337]  # می‌توانید شناسه یا نام کاربری تلگرام مدیران را اینجا وارد کنید
...
# تابعی برای بررسی اینکه آیا کاربر مجاز به اجرای دستورات است یا خیر
def is_admin(sender_id):
    return sender_id in admins or sender_id.username in admins
...
async def start_keep_alive(event):
    if not is_admin(await event.get_sender()):
        await event.reply("شما اجازه این کار را ندارید.")
        return
